...
var Util = (function () {
    ...
    Util.hasInterface = function (obj) {
        ...
        return Array.isArray(obj[FSymbol.interfaces])
            && obj[FSymbol.interfaces].some(function (x) { return interfaceNames.indexOf(x) >= 0; });
    };
    ...
    Util.equals = function (x, y) {
        if (x == null)
            ...
            return x.length != y.length
                ? false
                : Seq.fold2(function (prev, v1, v2) { return !prev ? prev : Util.equals(v1, v2); }, true, x, y);
    ...
    Util.compare = function (x, y) {
        if (x == null)
            ...
            return x.length != y.length
                ...
                : Seq.fold2(function (prev, v1, v2) { return prev !== 0 ? prev : Util.compare(v1, v2); }, 0, x, y);
    ...
    Util.createObj = function (fields) {
        return Seq.fold(function (acc, kv) { acc[kv[0]] = kv[1]; return acc; }, {}, fields);
    };
    return Util;
}());
        ...
        return JSON.stringify(o, function (k, v) {
            if (ArrayBuffer.isView(v)) {
                ...
                if (v instanceof List || v instanceof FSet || v instanceof Set) {
                    ...
                    return Seq.fold(function (o, kv) { o[kv[0]] = kv[1]; return o; }, { $type: v[FSymbol.typeName] || "System.Collections.Generic.Dictionary" }, v);
                }
                    ...
                    if (Util.hasInterface(v, "FSharpUnion", "FSharpRecord", "FSharpException")) {
                        ...
                        var proto = Object.getPrototypeOf(v), props = Object.getOwnPropertyNames(proto), o_1 = { $type: v[FSymbol.typeName] };
                        for (var i = 0; i < props.length; i++) {
                            var prop = Object.getOwnPropertyDescriptor(proto, props[i]);
                            if (prop.get)
...
var FDate = (function (_super) {
    ...
    FDate.create = function (year, month, day, h, m, s, ms, kind) {
        ...
        if (s === void 0) { s = 0; }
        if (ms === void 0) { ms = 0; }
        if (kind === void 0) { kind = DateKind.Local; }
        var date = (kind === DateKind.UTC)
...
var FString = (function () {
    ...
    FString.replicate = function (n, x) {
        return FString.initialize(n, function () { return x; });
    };
    FString.split = function (str, splitters, count, removeEmpty) {
        ...
        splitters = splitters.map(function (x) { return FRegExp.escape(x); });
...
var FArray = (function () {
    ...
    FArray.addRangeInPlace = function (range, xs) {
        Seq.iterate(function (x) { return xs.push(x); }, range);
    };
...
var List = (function () {
    ...
    Object.defineProperty(List.prototype, "length", {
        get: function () {
            return Seq.fold(function (acc, x) { return acc + 1; }, 0, this);
        },
        enumerable: true,
        configurable: true
    });
    ...
    List.append = function (xs, ys) {
        return Seq.fold(function (acc, x) { return new List(x, acc); }, ys, List.reverse(xs));
    };
    ...
    List.collect = function (f, xs) {
        return Seq.fold(function (acc, x) { return acc.append(f(x)); }, new List(), xs);
    };
    // TODO: should be xs: Iterable<List<T>>
    List.concat = function (xs) {
        return List.collect(function (x) { return x; }, xs);
    };
    List.prototype.filter = function (f) {
        return List.filter(f, this);
    };
    List.filter = function (f, xs) {
        return List.reverse(Seq.fold(function (acc, x) { return f(x) ? new List(x, acc) : acc; }, new List(), xs));
    };
    ...
    List.map = function (f, xs) {
        return List.reverse(Seq.fold(function (acc, x) { return new List(f(x), acc); }, new List(), xs));
    };
    List.prototype.mapIndexed = function (f) {
        return List.mapIndexed(f, this);
    };
    List.mapIndexed = function (f, xs) {
        return List.reverse(Seq.fold(function (acc, x, i) { return new List(f(i, x), acc); }, new List(), xs));
    };
    ...
    List.replicate = function (n, x) {
        return List.initialize(n, function () { return x; });
    };
    List.prototype.reverse = function () {
        return List.reverse(this);
    };
    List.reverse = function (xs) {
        return Seq.fold(function (acc, x) { return new List(x, acc); }, new List(), xs);
    };
    ...
    List.slice = function (lower, upper, xs) {
        ...
        return List.reverse(Seq.fold(function (acc, x, i) { return (noLower || lower <= i) && (noUpper || i <= upper) ? new List(x, acc) : acc; }, new List(), xs));
    };
    /* ToDo: instance unzip() */
    ...
    List.groupBy = function (f, xs) {
        return Seq.toList(Seq.map(function (k) { return Tuple(k[0], Seq.toList(k[1])); }, Seq.groupBy(f, xs)));
    };
    return List;
}());
exports.List = List;
...
var Seq = (function () {
    ...
    Seq.countBy = function (f, xs) {
        return Seq.map(function (kv) { return Tuple(kv[0], Seq.count(kv[1])); }, Seq.groupBy(f, xs));
    };
    ...
    Seq.compareWith = function (f, xs, ys) {
        var nonZero = Seq.tryFind(function (i) { return i != 0; }, Seq.map2(function (x, y) { return f(x, y); }, xs, ys));
    ...
    Seq.distinctBy = function (f, xs) {
        return Seq.choose(function (tup) { return tup[0]; }, Seq.scan(function (tup, x) {
            var acc = tup[1];
            var k = f(x);
    ...
    Seq.empty = function () {
        return Seq.unfold(function () { return void 0; });
    };
    Seq.enumerateWhile = function (cond, xs) {
        return Seq.concat(Seq.unfold(function () { return cond() ? [xs, true] : null; }));
    };
    ...
    Seq.except = function (itemsToExclude, source) {
        ...
        var testIsNotInExclusionItems = function (element) { return !exclusionItems.some(function (excludedItem) { return Util.equals(excludedItem, element); }); };
    ...
    Seq.filter = function (f, xs) {
        ...
        return Seq.delay(function () { return Seq.unfold(trySkipToNext, xs[Symbol.iterator]()); });
    };
    ...
    Seq.forAll = function (f, xs) {
        return Seq.fold(function (acc, x) { return acc && f(x); }, true, xs);
    };
    Seq.forAll2 = function (f, xs, ys) {
        return Seq.fold2(function (acc, x, y) { return acc && f(x, y); }, true, xs, ys);
    };
    ...
    Seq.groupBy = function (f, xs) {
        ...
        var map = Seq.fold(function (acc, x) {
            var k = f(x), vs = FMap.tryFind(k, acc);
            if (vs == null) {
                keys.push(k);
    ...
    Seq.initialize = function (n, f) {
        return Seq.delay(function () {
            return Seq.unfold(function (i) { return i < n ? [f(i), i + 1] : null; }, 0);
        });
    };
    Seq.initializeInfinite = function (f) {
        return Seq.delay(function () {
            return Seq.unfold(function (i) { return [f(i), i + 1]; }, 0);
        });
    };
    ...
    Seq.iterate = function (f, xs) {
        Seq.fold(function (_, x) { return f(x); }, null, xs);
    };
    Seq.iterate2 = function (f, xs, ys) {
        Seq.fold2(function (_, x, y) { return f(x, y); }, null, xs, ys);
    };
    Seq.iterateIndexed = function (f, xs) {
        Seq.fold(function (_, x, i) { return f(i, x); }, null, xs);
    };
    Seq.iterateIndexed2 = function (f, xs, ys) {
        Seq.fold2(function (_, x, y, i) { return f(i, x, y); }, null, xs, ys);
    };
    ...
    Seq.tryLast = function (xs) {
        try {
            return Seq.reduce(function (_, x) { return x; }, xs);
        }
        catch (err) {
            return null;
        }
    ...
    Seq.count = function (xs) {
        return Array.isArray(xs) || ArrayBuffer.isView(xs)
            ? xs.length
            : Seq.fold(function (acc, x) { return acc + 1; }, 0, xs);
    };
    Seq.map = function (f, xs) {
        return Seq.delay(function () { return Seq.unfold(function (iter) {
            var cur = iter.next();
    ...
    Seq.max = function (xs) {
        return Seq.reduce(function (acc, x) { return Util.compare(acc, x) === 1 ? acc : x; }, xs);
    };
    Seq.maxBy = function (f, xs) {
        return Seq.reduce(function (acc, x) { return Util.compare(f(acc), f(x)) === 1 ? acc : x; }, xs);
    };
    Seq.min = function (xs) {
        return Seq.reduce(function (acc, x) { return Util.compare(acc, x) === -1 ? acc : x; }, xs);
    };
    ...
    Seq.pairwise = function (xs) {
        return Seq.skip(2, Seq.scan(function (last, next) { return Tuple(last[1], next); }, Tuple(0, 0), xs));
    };
    ...
    Seq.rangeStep = function (first, step, last) {
        ...
        return Seq.delay(function () { return Seq.unfold(function (x) { return step > 0 && x <= last || step < 0 && x >= last ? [x, x + step] : null; }, first); });
    };
    Seq.rangeChar = function (first, last) {
        return Seq.delay(function () { return Seq.unfold(function (x) { return x <= last ? [x, String.fromCharCode(x.charCodeAt(0) + 1)] : null; }, first); });
    };
    ...
    Seq.replicate = function (n, x) {
        return Seq.initialize(n, function () { return x; });
    };
    ...
    Seq.scanBack = function (f, xs, seed) {
        return Seq.reverse(Seq.scan(function (acc, x) { return f(x, acc); }, seed, Seq.reverse(xs)));
    };
    Seq.singleton = function (x) {
        return Seq.unfold(function (x) { return x != null ? [x, null] : null; }, x);
    };
    ...
    Seq.skipWhile = function (f, xs) {
        return Seq.delay(function () {
            var hasPassed = false;
            return Seq.filter(function (x) { return hasPassed || (hasPassed = !f(x)); }, xs);
        });
    };
    ...
    Seq.sum = function (xs) {
        return Seq.fold(function (acc, x) { return acc + x; }, 0, xs);
    };
    Seq.sumBy = function (f, xs) {
        return Seq.fold(function (acc, x) { return acc + f(x); }, 0, xs);
    };
    ...
    Seq.zip = function (xs, ys) {
        return Seq.map2(function (x, y) { return [x, y]; }, xs, ys);
    };
    Seq.zip3 = function (xs, ys, zs) {
        return Seq.map3(function (x, y, z) { return [x, y, z]; }, xs, ys, zs);
    };
    return Seq;
}());
exports.Seq = Seq;
var SetTree = (function () {
    ...
    SetTree.balance = function (comparer, t1, k, t2) {
        ...
        var $target1 = function (t1_1) { return SetTree.add(comparer, k, t1_1); };
        var $target2 = function (k1, t2_1) { return SetTree.add(comparer, k, SetTree.add(comparer, k1, t2_1)); };
    ...
    SetTree.subset = function (comparer, a, b) {
        return SetTree.forall(function (x) { return SetTree.mem(comparer, x, b); }, a);
    };
    SetTree.psubset = function (comparer, a, b) {
        return SetTree.forall(function (x) { return SetTree.mem(comparer, x, b); }, a) ? SetTree.exists(function (x) { return !SetTree.mem(comparer, x, a); }, b) : false;
    };
    ...
    SetTree.union = function (comparer, t1, t2) {
        ...
        var $target3 = function (k1, t2_1) { return SetTree.add(comparer, k1, t2_1); };
        if (matchValue[0].Case === "SetEmpty") {
            var t = matchValue[1];
            return t;
        }
    ...
    SetTree.compareStacks = function (comparer, l1, l2) {
        var $target8 = function (n1k, t1) { return SetTree.compareStacks(comparer, List.ofArray([new SetTree("SetEmpty", []), SetTree.SetOne(n1k)], t1), l2); };
        var $target9 = function (n1k, n1l, n1r, t1) { return SetTree.compareStacks(comparer, List.ofArray([n1l, SetTree.SetNode(n1k, new SetTree("SetEmpty", []), n1r, 0)], t1), l2); };
        var $target11 = function (n2k, n2l, n2r, t2) { return SetTree.compareStacks(comparer, l1, List.ofArray([n2l, SetTree.SetNode(n2k, new SetTree("SetEmpty", []), n2r, 0)], t2)); };
        if (l1.tail != null) {
            if (l2.tail != null) {
...
var FSet = (function () {
    ...
    FSet.prototype[Symbol.iterator] = function () {
        ...
        return {
            next: function () { return SetTree.moveNext(i); }
        };
    };
    ...
    FSet.unionMany = function (sets) {
        ...
        return Seq.fold(function (acc, s) { return FSet.union(s, acc); }, FSet.create(), sets);
    };
    ...
    FSet.intersectMany = function (sets) {
        return Seq.reduce(function (s1, s2) { return FSet.intersect(s1, s2); }, sets);
    };
    FSet.isProperSubsetOf = function (set1, set2) {
        if (set1 instanceof FSet && set2 instanceof FSet) {
            ...
            return Seq.forAll(function (x) { return set2.has(x); }, set1) && Seq.exists(function (x) { return !set1.has(x); }, set2);
        }
    };
    FSet.isSubsetOf = function (set1, set2) {
        if (set1 instanceof FSet && set2 instanceof FSet) {
            ...
            set2 = set2 instanceof Set ? set2 : new Set(set2);
            return Seq.forAll(function (x) { return set2.has(x); }, set1);
        }
    };
    ...
    FSet.map = function (f, s) {
        ...
        return FSet.from(comparer, SetTree.fold(function (acc, k) { return SetTree.add(comparer, f(k), acc); }, new SetTree("SetEmpty", []), s.tree));
    };
...
var FMap = (function () {
    ...
    FMap.prototype[Symbol.iterator] = function () {
        ...
        return {
            next: function () { return MapTree.moveNext(i); }
        };
    };
    ...
    FMap.prototype.keys = function () {
        return Seq.map(function (kv) { return kv[0]; }, this);
    };
    FMap.prototype.values = function () {
        return Seq.map(function (kv) { return kv[1]; }, this);
    };
    ...
    FMap.containsValue = function (v, map) {
        return Seq.fold(function (acc, k) { return acc || Util.equals(map.get(k), v); }, false, map.keys());
    };
    ...
    FMap.findKey = function (f, map) {
        return Seq.pick(function (kv) { return f(kv[0], kv[1]) ? kv[0] : null; }, map);
    };
    FMap.tryFindKey = function (f, map) {
        return Seq.tryPick(function (kv) { return f(kv[0], kv[1]) ? kv[0] : null; }, map);
    };
...
var AsyncImpl = {
    ...
    GAS_return: function (value) {
        return AsyncImpl.protectedCont(function (ctx) { return ctx.onSuccess(value); });
    }
};
var AsyncBuilder = (function () {
    ...
    AsyncBuilder.prototype.Delay = function (generator) {
        return AsyncImpl.protectedCont(function (ctx) { return generator()(ctx); });
    };
    ...
    AsyncBuilder.prototype.While = function (guard, computation) {
        ...
        if (guard())
            ...
            return this.Return(exports.Nothing);
    };
    AsyncBuilder.prototype.Zero = function () {
        return AsyncImpl.protectedCont(function (ctx) { return ctx.onSuccess(exports.Nothing); });
    };
    return AsyncBuilder;
}());
...
var Async = (function () {
    ...
    Object.defineProperty(Async, "cancellationToken", {
        get: function () {
            return AsyncImpl.protectedCont(function (ctx) { return ctx.onSuccess(ctx.cancelToken); });
        },
        enumerable: true,
        configurable: true
    });
    Async.GAS_catch = function (work) {
        return AsyncImpl.protectedCont(function (ctx) {
            work({
                onSuccess: function (x) { return ctx.onSuccess(Choice.Choice1Of2(x)); },
                onError: function (ex) { return ctx.onSuccess(Choice.Choice2Of2(ex)); },
                onCancel: ctx.onCancel,
                cancelToken: ctx.cancelToken,
                trampoline: ctx.trampoline
            });
    ...
    Async.fromContinuations = function (f) {
        return AsyncImpl.protectedCont(function (ctx) { return f([ctx.onSuccess, ctx.onError, ctx.onCancel]); });
    };
    Async.ignore = function (computation) {
        return AsyncImpl.bind(computation, function (x) { return AsyncImpl.GAS_return(exports.Nothing); });
    };
    Async.parallel = function (computations) {
        return Async.awaitPromise(Promise.all(Seq.map(function (w) { return Async.startAsPromise(w); }, computations)));
    };
    Async.sleep = function (millisecondsDueTime) {
        return AsyncImpl.protectedCont(function (ctx) {
            setTimeout(function () { return ctx.cancelToken.isCancelled ? ctx.onCancel("cancelled") : ctx.onSuccess(exports.Nothing); }, millisecondsDueTime);
        });
    };
...
var FObservable = (function () {
    ...
    FObservable.choose = function (chooser, source) {
        return new Observable(function (observer) {
            return source.Subscribe(new Observer(function (t) {
                return FObservable.__protect(function () { return chooser(t); }, function (u) { if (u != null)
                    observer.OnNext(u); }, observer.OnError);
            }, observer.OnError, observer.OnCompleted));
        });
    };
    FObservable.filter = function (predicate, source) {
        return FObservable.choose(function (x) { return predicate(x) ? x : null; }, source);
    };
    FObservable.map = function (mapping, source) {
        return new Observable(function (observer) {
            return source.Subscribe(new Observer(function (t) {
                FObservable.__protect(function () { return mapping(t); }, observer.OnNext, observer.OnError);
            }, observer.OnError, observer.OnCompleted));
        });
    };
    ...
    FObservable.partition = function (predicate, source) {
        return Tuple(FObservable.filter(predicate, source), FObservable.filter(function (x) { return !predicate(x); }, source));
    };
    FObservable.scan = function (collector, state, source) {
        return new Observable(function (observer) {
            return source.Subscribe(new Observer(function (t) {
                FObservable.__protect(function () { return collector(state, t); }, function (u) { state = u; observer.OnNext(u); }, observer.OnError);
            }, observer.OnError, observer.OnCompleted));
        });
    };
    FObservable.split = function (splitter, source) {
        return Tuple(FObservable.choose(function (v) { return splitter(v).valueIfChoice1; }, source), FObservable.choose(function (v) { return splitter(v).valueIfChoice2; }, source));
    };
...
var Event = (function () {
    ...
    Event.prototype.Trigger = function (value) {
        Seq.iterate(function (f) { return f(value); }, this.delegates);
    };
    ...
    Event.prototype._removeHandler = function (f) {
        var index = this.delegates.findIndex(function (el) { return "" + el == "" + f; }); // Special dedication to Chet Husk.
        if (index > -1)
            this.delegates.splice(index, 1);
    };
    Event.prototype.AddHandler = function (handler) {
        this._addHandler(function (x) { return handler(undefined, x); });
    };
    Event.prototype.RemoveHandler = function (handler) {
        this._removeHandler(function (x) { return handler(undefined, x); });
    };
    ...
    Event.prototype._subscribeFromObserver = function (observer) {
        ...
        return Util.createDisposable(function () { return _this._removeHandler(callback); });
    };
    ...
    Event.filter = function (predicate, sourceEvent) {
        return Event.choose(function (x) { return predicate(x) ? x : null; }, sourceEvent);
    };
    Event.map = function (mapping, sourceEvent) {
        var source = sourceEvent;
        return new Event(function (observer) {
            return source.Subscribe(new Observer(function (t) {
                return FObservable.__protect(function () { return mapping(t); }, observer.OnNext, observer.OnError);
            }, observer.OnError, observer.OnCompleted));
        }, source.delegates);
    };
    ...
    Event.partition = function (predicate, sourceEvent) {
        return Tuple(Event.filter(predicate, sourceEvent), Event.filter(function (x) { return !predicate(x); }, sourceEvent));
    };
    ...
    Event.split = function (splitter, sourceEvent) {
        return Tuple(Event.choose(function (v) { return splitter(v).valueIfChoice1; }, sourceEvent), Event.choose(function (v) { return splitter(v).valueIfChoice2; }, sourceEvent));
    };
    return Event;
}());
exports.Event = Event;
